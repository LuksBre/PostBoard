...
export default function DetalhesScreen({ navigation, route }) {
  ...
  const [erroAutor, setErroAutor] = useState(false); // ✅ CERTO
  ...
  useEffect(() => {
    async function carregarAutor() {
      try {
        ...
        // ❌ era console.warn → agora vira erro visual
        setErroAutor(true);
      } finally {
        setLoading(false);
      }
    ...
    <ScrollView style={styles.container}>
      ...
      {erroAutor && (
        <Text style={{
          ...
          Não foi possível carregar as informações do autor.
        </Text>
      )}
...
const styles = StyleSheet.create({
  ...
  autorLabel: { fontSize: 11, color: '#9ca3af', fontWeight: '700',
